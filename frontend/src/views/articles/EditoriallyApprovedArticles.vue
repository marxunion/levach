<script setup lang="ts">
    ...
    const fetchNewArticles = async (count : number = 4) => 
    {
        ...
        if(searchTitle.length > 0 && searchTags.length > 0)
        {
            params = {
                ...
                category: 'EditoriallyApprovedArticles',
                count: count,
    ...
    {
        articles.value = [];
        LangDataHandler.destroyLangDataHandler('EditoriallyApprovedArticles');
    });
...
<template>
    <main class="main">
        <div class="main__header">
			<p class="main__header__title">{{ (langData['headerTitle'] as JsonData)['EditoriallyApprovedArticles'] }}</p>
