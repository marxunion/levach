	...
	const onSendButton = async () =>
	{
        ...
        else
        {
            ...
            .then(async response => 
            {
                if(response.data.success)
                {
                    const modal = await openModal(InfoModalWithLink, {status: true, text: langData.value['articleApprovedSuccessfully'], link: window.location.hostname + "/article/edit/" + articleViewCode.value, textFooter: (langData.value['warnings'] as JsonData)['articleEditLinkCopyWarning']});
