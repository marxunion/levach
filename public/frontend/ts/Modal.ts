class ModalDialogSwitch {
  ...
  private modal_content: HTMLElement;
  private modal_closer: HTMLElement;
  private elements_lock_paddings: HTMLElement[];
  private scrollWidth: number;
  private body: HTMLElement;
  private modal_content_className: string;

  constructor(
    modal: HTMLElement,
    modal_content: HTMLElement,
    modal_closer: HTMLElement,
    elements_lock_paddings: HTMLElement[],
    scrollWidth: number = window.innerWidth - (document.querySelector('body') as HTMLElement).clientWidth
  ) {
    this.modal = modal;
  ...
  private _closeDialog(timeout_modal_closed: number, class_name_lock_padding: string): void {
  ...
  public activateDialog(
    timeout_modal_content_activate: number = 100,
    timeout_modal_closed: number = 700,
    class_name_lock_padding: string = 'lock-padding-modal'
  ): void {
    ...
    setTimeout(() => {
      this.modal.addEventListener('click', (e) => {
        if (!(e.target as HTMLElement).classList.contains(this.modal_content_className)) {
